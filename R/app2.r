...
# Helper function to recalculate values for filtered hierarchy
recalculate_hierarchy_values <- function(filtered_data, leaf_node_ids) {
  # Set all leaf nodes to 1
  filtered_data$values <- ifelse(filtered_data$ids %in% leaf_node_ids, 1, filtered_data$values)
  
  # Recalculate parent node values to be the sum of their children
  # This ensures proper proportions for branchvalues = "remainder"
  ...
  # Go from deepest to shallowest level
