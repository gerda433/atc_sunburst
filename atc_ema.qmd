...
```{r}
ema_medicines_raw <- medicines_output_medicines_en_2 <- read_excel("medicines data/medicines_output_medicines_en-2.xlsx", skip = 8)
...
ema_medicines <- ema_medicines_names %>%
  ...
  mutate(drug_information = paste0("<b>Indication(s)</b><Br>", str_replace_all(therapeutic_area, ";", "<Br>"), "<Br><Br><b>Date authorised</b><Br>", marketing_authorisation_date))
...
# just trying out hovertemplate, which is interesting, but not useful right here
p2 <- plot_ly(
  sunburst_df,
  ids = ~ids,
  labels = ~labels,
...
# Search by text only
select_category_level(hierarchy, text_input = "bcr")

# Search by ATC code only
select_category_level(hierarchy, atc_input = "J07BB")
