...
Next, I need to rearrange these data:
...
as.sunburstDF_plotly <- function(DF_plotly, value_column = NULL, add_root = FALSE, drop_na_nodes = TRUE){
...
library(data.table)
hierarchyDF <- as.sunburstDF_plotly(df, value_column = "value", add_root = TRUE)

# the next step is a little unusual, probably a better solution out there:
hierarchyDF_text <- as.sunburstDF_plotly(df_text, value_column = "value", add_root = TRUE) %>%
  mutate(
...
```{r}
atc_ema <- atc_levels %>%
  left_join(ema_medicines %>% select(atc_code_human, name_of_medicine), join_by(atc_code == atc_code_human)) 
```
